...
import { IReader, IObservable, IObserver } from "base/common/observableImpl/base";
import { getLogger } from "base/common/observableImpl/logging";
...
export function autorunWithStore(fn: (reader: IReader, store: DisposableStore) => void, debugName: string): IDisposable {
...
export class AutorunObserver implements IObserver, IReader, IDisposable {
  ...
  constructor(public readonly debugName: string, private readonly runFn: (reader: IReader) => void, private readonly _handleChange: ((context: IChangeContext) => boolean) | undefined) {
...
export function autorunDelta<T>(name: string, observable: IObservable<T>, handler: (args: { lastValue: T | undefined; newValue: T }) => void): IDisposable {
