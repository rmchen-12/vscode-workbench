...
import { IReader, IObservable, BaseObservable, IObserver, _setDerived } from "base/common/observableImpl/base";
...
export class Derived<T> extends BaseObservable<T, void> implements IReader, IObserver {
  ...
  constructor(private readonly _debugName: string | (() => string), private readonly computeFn: (reader: IReader) => T) {
    super();
  ...
  public handleChange<T, TChange>(_observable: IObservable<T, TChange>, _change: TChange): void {
    if (this.hasValue) {
      this.hadValue = true;
      this.hasValue = false;
    }
