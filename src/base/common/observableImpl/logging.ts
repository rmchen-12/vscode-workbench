...
import { AutorunObserver } from "base/common/observableImpl/autorun";
import { IObservable, ObservableValue, TransactionImpl } from "base/common/observableImpl/base";
import { Derived } from "base/common/observableImpl/derived";
import { FromEventObservable } from "base/common/observableImpl/utils";
...
export class ConsoleObservableLogger implements IObservableLogger {
  ...
  handleDerivedRecomputed(derived: Derived<unknown>, info: IChangeInformation): void {
    ...
    console.log(...this.textToConsoleArgs([formatKind("derived recomputed"), styled(derived.debugName, { color: "BlueViolet" }), ...this.formatInfo(info), this.formatChanges(changedObservables)]));
    changedObservables.clear();
  }

  handleFromEventObservableTriggered(observable: FromEventObservable<any, any>, info: IChangeInformation): void {
    console.log(...this.textToConsoleArgs([formatKind("observable from event triggered"), styled(observable.debugName, { color: "BlueViolet" }), ...this.formatInfo(info)]));
  }
  ...
  handleAutorunTriggered(autorun: AutorunObserver): void {
    ...
    console.log(...this.textToConsoleArgs([formatKind("autorun"), styled(autorun.debugName, { color: "BlueViolet" }), this.formatChanges(changedObservables)]));
    changedObservables.clear();
  }
  ...
  handleBeginTransaction(transaction: TransactionImpl): void {
    ...
    console.log(...this.textToConsoleArgs([formatKind("transaction"), styled(transactionName, { color: "BlueViolet" })]));
    this.indentation++;
  }
...
type ConsoleText = (ConsoleText | undefined)[] | { text: string; style: string; data?: Record<string, unknown> } | { data: Record<string, unknown> };
